...
func TestListExpensesPaginationContract(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedLimit: 20,
			expectedPages: 3,
			expectedArgs:  []any{testExpenseAccountID, 20, 0},
		},
		{
			name:          "caps limit at max and applies offset",
			path:          "/expenses?page=2&limit=150",
			totalCount:    250,
			expectedPage:  2,
			expectedLimit: 100,
			expectedPages: 3,
			expectedArgs:  []any{testExpenseAccountID, 100, 100},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)SELECT e\.id.*LIMIT \$2 OFFSET \$3`).
				WithArgs(tt.expectedArgs...).
				WillReturnRows(expenseListRows(mock))
			...
			if len(response.Expenses) != 0 {
				t.Fatalf("len(expenses) = %d, want 0", len(response.Expenses))
			}
...
func TestListExpensesPaginationPreservesFilterArgs(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	args := []any{testExpenseAccountID, "2026-01-01", "2026-01-31", "one-time", categoryID, familyMemberID}

	mock.ExpectQuery(`SELECT COUNT\(\*\) FROM expenses e WHERE`).
		WithArgs(args...).
		WillReturnRows(mock.NewRows([]string{"count"}).AddRow(101))
	mock.ExpectQuery(`(?s)SELECT e\.id.*ORDER BY e\.amount ASC.*LIMIT \$7 OFFSET \$8`).
		WithArgs(append(args, 10, 20)...).
		WillReturnRows(expenseListRows(mock))

	recorder := httptest.NewRecorder()
	router := expenseTestRouter(listExpensesHandler(mock))
	req := httptest.NewRequest(http.MethodGet, "/expenses?date_from=2026-01-01&date_to=2026-01-31&expense_type=one-time&category_id="+categoryID+"&family_member_id="+familyMemberID+"&sort_by=amount&order=asc&page=3&limit=10", nil)
	router.ServeHTTP(recorder, req)
...
func expenseListRows(mock pgxmock.PgxPoolIface) *pgxmock.Rows {
	return mock.NewRows([]string{"id", "family_member_id", "category_id", "category_name", "description", "amount", "currency", "exchange_rate", "amount_in_primary_currency", "expense_type", "date", "end_date", "payment_method", "source_container_id", "source_instrument_id", "container_name", "container_type", "instrument_name", "instrument_type", "created_at"})
}
