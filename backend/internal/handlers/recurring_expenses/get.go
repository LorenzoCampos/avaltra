...
func GetRecurringExpense(pool *pgxpool.Pool) gin.HandlerFunc {
	return getRecurringExpenseHandler(pool)
}

func getRecurringExpenseHandler(pool recurringExpenseStore) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var categoryID, categoryName, familyMemberID, familyMemberName, sourceContainerID, sourceInstrumentID *string
		...
		detail.SourceContainerID = sourceContainerID
		detail.SourceInstrumentID = sourceInstrumentID

		// Convertir dates a string
		if startDate != nil {
			detail.StartDate = recurringExpenseDateString(startDate)
		}

		if endDate != nil {
			endDateStr := recurringExpenseDateString(endDate)
...
func recurringExpenseDateString(value interface{}) string {
