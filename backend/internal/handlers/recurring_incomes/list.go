...
// RecurringIncomeListItem representa un item en la lista
type RecurringIncomeListItem struct {
	ID                      string   `json:"id"`
...
// ListRecurringIncomes maneja GET /api/recurring-expenses
func ListRecurringIncomes(pool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		baseQuery := `
			SELECT 
				...
				re.created_at
			FROM recurring_incomes re
			LEFT JOIN income_categories ec ON re.category_id = ec.id
		...
		// Parsear resultados
		recurringExpenses := make([]RecurringIncomeListItem, 0)

		for rows.Next() {
			var item RecurringIncomeListItem
			var categoryName, familyMemberName, destinationContainerID, destinationInstrumentID *string
			...
			item.DestinationContainerID = destinationContainerID
			item.DestinationInstrumentID = destinationInstrumentID
		...
		c.JSON(http.StatusOK, gin.H{
			"recurring_incomes": recurringExpenses,
