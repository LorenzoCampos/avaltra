...
export const ExpenseList = () => {
  ...
  const { expenses, expensesPagination, isLoadingExpenses, expensesError, deleteExpenseAsync, restoreExpenseAsync, isDeletingExpense, isRestoringExpense } = useExpenses({ page, limit: PAGE_SIZE });
  ...
  const pagination = getListPaginationState(expensesPagination);
  ...
  const handleDelete = (e: React.MouseEvent, expense: Expense) => {
    ...
    handleDeleteWithAnimation(expense.id, async () => {
      ...
      const toastId = toast.success(t('list.toast.deleted', { description: expense.description }), {
        duration: 5000,
        action: {
          label: t('list.toast.undo'),
          onClick: async () => {
            try {
              await restoreExpenseAsync(expense);
      ...
      {pagination.total_count === 0 ? (
        <Card>
          <CardContent>
            <EmptyState
              icon="📝"
              title={t('common:emptyState.expenses.title')}
              description={t('common:emptyState.expenses.description')}
              action={{
                label: t('common:emptyState.expenses.action'),
            ...
            <CardHeader>
              <CardTitle>
                {activeFiltersCount > 0 ? (
                  ...
                  <span>{t('list.allExpenses')} ({pagination.total_count})</span>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      <tr 
                        ...
                        className={cn(
                          ...
                          isDeleting(expense.id) ? 'animate-feedback-exit-fast' : getFeedbackClassName(expense.id)
                        )}
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                          ...
                          <button
                            ...
                            disabled={isDeletingExpense || isRestoringExpense}
                          >
