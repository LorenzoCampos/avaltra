...
import { paymentContainerKindLabels } from '@/types/paymentContainer';
import type { PaymentInstrument } from '@/types/paymentInstrument';
import { paymentInstrumentKindLabels } from '@/types/paymentInstrument';
...
export function PaymentContainersPage() {
  ...
  if (isLoading) {
    return <div className="rounded-2xl bg-white p-6 text-gray-600 shadow-sm dark:bg-gray-900 dark:text-gray-300">Loading payment context…</div>;
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">
        Failed to load payment containers and instruments. Please try again.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <header>
        <p className="text-sm font-semibold uppercase tracking-wide text-brand-primary">Payment context</p>
        <h1 className="mt-1 text-3xl font-bold text-gray-900 dark:text-white">Containers and instruments</h1>
        <p className="mt-2 max-w-3xl text-gray-600 dark:text-gray-300">
          Manage where money is held and how it moves. Transaction forms keep using the legacy payment method until the next slice.
        </p>
      </header>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-3 border-b border-gray-200 p-5 dark:border-gray-800">
            ...
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-white">Containers</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Active containers appear in new selection lists.</p>
            </div>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {containers.length === 0 ? (
              <p className="p-5 text-sm text-gray-500 dark:text-gray-400">No containers yet.</p>
            ) : (
              containers.map((container) => (
                <div key={container.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">{paymentContainerKindLabels[container.kind]}</p>
                  </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-3 border-b border-gray-200 p-5 dark:border-gray-800">
            ...
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-white">Instruments</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Cards require a backing container.</p>
            </div>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {instruments.length === 0 ? (
              <p className="p-5 text-sm text-gray-500 dark:text-gray-400">No instruments yet.</p>
            ) : (
                ...
                <div key={instrument.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {paymentInstrumentKindLabels[instrument.kind]}
                      {instrument.backing_container_id ? ` backed by ${containerNames.get(instrument.backing_container_id) ?? 'unknown container'}` : ''}
                    </p>
                  </div>
                  <div className="flex gap-2">
