...
import { useRecurringExpenses, useDeleteRecurringExpense, useUpdateRecurringExpense } from '@/hooks/useRecurringExpenses';
...
export const RecurringExpensesList = () => {
  ...
  const formatRecurrenceLabel = (item: RecurringExpense): string => {
    ...
    switch (recurrence_frequency) {
      ...
      case 'weekly': {
        const day = recurrence_day_of_week !== undefined && recurrence_day_of_week !== null 
          ? t(`expenses.daysOfWeek.${recurrence_day_of_week}`) 
          : '';
  ...
  const { data, isLoading, error } = useRecurringExpenses({ 
    is_active: activeFilter,
    frequency: frequencyFilter === 'all' ? undefined : frequencyFilter
  });
  
  const { mutate: deleteRecurringExpense, isPending: isDeleting } = useDeleteRecurringExpense();
  const { mutate: updateRecurringExpense, isPending: isUpdating } = useUpdateRecurringExpense();
  ...
  // Calculate summary
  const activeExpenses = useMemo(() => recurringExpenses.filter(e => e.is_active), [recurringExpenses]);
  ...
  const handleEdit = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    navigate(`/expenses/recurring/edit/${id}`);
  };

  const handleToggleActive = (e: React.MouseEvent, expense: RecurringExpense) => {
    e.stopPropagation();
  ...
  const handleDelete = (e: React.MouseEvent, id: string, description: string) => {
    ...
    if (window.confirm(t('common:messages.confirmDelete', { entity: description }))) {
      handleDeleteWithAnimation(id, () => deleteRecurringExpense(id));
    }
  };
    ...
    return (
      <div className="text-center py-10">
        <div className="p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
          <h3 className="text-lg font-medium text-red-800 dark:text-red-400 mb-2">{t('common:errors.errorLoading', { entity: t('expenses.title') })}</h3>
      ...
      {recurringExpenses.length === 0 ? (
        ...
        <>
          ...
          <Card className="hidden md:block">
            <CardHeader>
              <CardTitle>
                {activeFilter === 'true' ? t('common.active') : activeFilter === 'false' ? t('common.inactive') : t('common:filters.all')} {t('expenses.title')} ({total})
              </CardTitle>
            </CardHeader>
            <CardContent>
